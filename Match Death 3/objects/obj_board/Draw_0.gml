...
with(obj_tile){
	if(!isSelected){
		draw_sprite_ext(spr_tiles, tileType, x, y, 1 + 0.1*hoverScale + 1*playingCounter, 1 + 0.1*hoverScale + 1*playingCounter, tileAngle + 6*power(dcos(playingCounter*1200), 3), c_white, 1 - (1 - point_distance(tileGridPos[0], tileGridPos[1], tileGamePos[0], tileGamePos[1]))*0.3*isReadyForPlay*(1 + 0.5*dcos(current_time)));
	}		
}
with(obj_enemy){
	draw_sprite_ext(spr_monsters, enemyType, x, y, 1 + 0.1*hoverScale, 1 + 0.1*hoverScale, enemyAngle, c_white, 1);
}
