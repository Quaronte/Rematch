...
with(obj_tile){
	...
	if(tileGroup != -1 && hoverCounter > 0){
		...
		with(_currentGroup){
			var _gridOffset = groupGridHighlights == obj_board.playGrid ? [0, 0] : obj_board.enemyGridOffset;
			...
			for(var i = 0; i < array_length(_currentGroupHighlightsMap); i++){
				...
				draw_sprite_ext(spr_attackEffects, groupType, ((_currentGroupHighlightsArray[0]) + _gridOffset[0]) * sprite_get_width(spr_tiles), (_currentGroupHighlightsArray[1]) * sprite_get_height(spr_tiles), 1, 1, other.tileAngle, c_white, other.hoverCounter*0.7*(1 + 0.5*dcos(current_time)));
			}
		}
	}
}
...
draw_text(-2*sprite_get_width(spr_tiles), 7*sprite_get_height(spr_tiles), "Draw Pile: " + string(ds_list_size(playingDeck)));
draw_text(-2*sprite_get_width(spr_tiles), 7.5*sprite_get_height(spr_tiles), "Discard Pile: " + string(ds_list_size(discardedDeck)));
draw_text(-2*sprite_get_width(spr_tiles), 8*sprite_get_height(spr_tiles), "Swaps Left: " + string(remainingMoves) + "/" + string(movesPerTurn));


var mouseGridPosition = GetGridMousePosition();
var textDebugPos = [64*11, -20];
...
if(obj_game.debugMode){
	...
	draw_text(textDebugPos[0], textDebugPos[1], "Grid element: " + string_replace(string(cellContent), "ref instance 10", ""));
	textDebugPos[1] += 30;
